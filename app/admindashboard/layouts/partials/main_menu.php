  ...
  <div class="header__container container">
    <nav class="header__nav navbar navbar-expand-lg bg-transparent" aria-labelledby="header-navigation">
      <div class="container-fluid">
        <a href="<? ROOT ?>/home" class="nav-brand">
        ...
        <div class="collapse navbar-collapse" id="main_menu">
          <ul class="header__nav-list navbar-nav mb-2 mb-lg-0 align-items-center column-gap-4">
            <?php foreach ($menu as $key => $val):
              $active = ''; ?>
              <?php if (is_array($val)): ?>
                <li class="header__list-item nav-item dropdown">
                  <a class="header__item-link nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false"><?= $key ?></a>
                  <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                    <?php foreach ($val as $k => $v):
                      $active = ($v == $currentPage) ? 'active' : '' ?>
                      <?php if ($k == 'separator'): ?>
                        <li class="header__list-item dropdown-divider" role="separator"></li>
                      <?php else: ?>
                        <li class="header__list-item"><a class="<?= $active ?> dropdown-item" href="<?= $v ?>"><?= $k ?></a></li>
                      <?php endif; ?>
                    <?php endforeach; ?>
                  </ul>
                </li>
              ...
              <?php else:
                ...
                <li class="header__list-item"><a class="<?= $active ?> header__item-link dropdown-item" href="<?= $val ?>"><?= $key ?></a></li>
              <?php endif; ?>
            <?php endforeach; ?>
          </ul>
          <ul class="nav navbar-nav navbar-right">
            <?php if (Users::currentUser()): ?>
              <li class="header__list-item"><a href="#" class="nav__username">Hello, <?= Users::currentUser()->fname ?></a></li>
            <?php endif; ?>
          </ul>
        </div>
      </div>
